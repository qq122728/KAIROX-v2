...
function KycPage({ kycStatus, rejectedReason, setKycStatus, push, done }: { kycStatus: string; rejectedReason?: string | null; setKycStatus: (v: "none" | "pending" | "approved" | "rejected") => void; push: (p: StackPage) => void; done: () => void }) {
  ...
  async function uploadSide(file: File, side: "front" | "back") {
    ...
    if (file.size > RAW_MAX) { setErr("File exceeds 25MB limit"); setStatus("error"); return; }
    if (!SUPPORTED.has(file.type)) { setErr("Unsupported image format"); setStatus("error"); return; }
    ...
    try {
      ...
      if (!res.ok) {
        const msg = typeof data.error === "string" ? data.error : "Upload failed, please retry";
      ...
      if (data.uploadToken) {
        ...
        setErr("Upload failed, please retry"); setStatus("error");
      }
    } catch (e: any) {
      if (e?.name === "AbortError") return;
      setErr("Network error, please retry"); setStatus("error");
    }
  }
  ...
  async function submit() {
    if (legalName.trim().length <= 1) { setError("Please enter your legal name"); return; }
    if (!frontToken || !backToken) { setError("Please upload both sides of your document"); return; }
    setError("");
    setSubmitting(true);
    try {
      ...
      if (!res.ok) {
        const msg = typeof payload?.error === "string" ? payload.error : "Submission failed, please retry";
        setError(msg); return;
      }
      ...
      <div className="kyc-field">
        ...
        {status === "idle" && (
          <label className="kyc-upload">
            ...
            <span className="kyc-upload-main">Tap to upload{label}</span>
            <span className="kyc-upload-sub">Supports JPG, PNG, WebP, HEIC, up to 25MB</span>
          </label>
        )}
        {status === "uploading" && (
          <div className="kyc-upload kyc-upload-loading">
            ...
            <span className="kyc-upload-main">Uploading...</span>
            <span className="kyc-upload-sub">{name || "Please wait"}</span>
          </div>
        )}
        {status === "done" && token && (
          <div className="kyc-upload kyc-upload-done">
            ...
            <span className="kyc-upload-sub">Upload successful</span>
            <button type="button" className="kyc-retry-btn" onClick={() => clearSide(side)}>Change</button>
          </div>
        )}
        {status === "error" && (
          <div className="kyc-upload kyc-upload-error">
            ...
            <span className="kyc-upload-main">Upload failed</span>
            <span className="kyc-upload-sub">{errMsg || "Please retry"}</span>
            <button type="button" className="kyc-retry-btn" onClick={() => retrySide(side)}>Retry</button>
          </div>
        )}
      </div>
    );
