...
export async function PATCH(request: Request) {
  try {
    ...
    const body = await readJson<{ orderId: number; result: "won" | "lost" | "draw"; settlePrice?: number; note?: string }>(request);
    if (!body.orderId) return badRequest("Missing order");
    if (body.result !== "won" && body.result !== "lost" && body.result !== "draw") return badRequest("Invalid settlement result");
